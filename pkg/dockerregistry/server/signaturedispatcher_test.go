...
func TestSingatureGet(t *testing.T) {
...
func TestSingaturePut(t *testing.T) {
	client := &testclient.Fake{}
	// TODO: get rid of those nasty global vars
	backupRegistryClient := DefaultRegistryClient
	DefaultRegistryClient = makeFakeRegistryClient(client, fake.NewSimpleClientset())
	...
	if !reflect.DeepEqual(&testSignature, newImageSignature) {
		t.Fatalf("unexpected signature: %#+v", newImageSignature)
	}
}
