...
const (
	...
	// EnforceAcceptSchema2EnvVar is a boolean environment variable that allows to turn on schema version 2.
	EnforceAcceptSchema2EnvVar = "REGISTRY_MIDDLEWARE_REPOSITORY_OPENSHIFT_ACCEPTSCHEMA2"
)
...
func newRepositoryWithClient(
	...
	var acceptschema2 bool

	if os.Getenv(EnforceAcceptSchema2EnvVar) == "" {
...
func (r *repository) fillImageWithMetadata(manifest distribution.Manifest, image *imageapi.Image) error {
	...
	for i := range image.DockerImageLayers {
		// DockerImageLayers represents manifest.Manifest.Layers in reversed order
